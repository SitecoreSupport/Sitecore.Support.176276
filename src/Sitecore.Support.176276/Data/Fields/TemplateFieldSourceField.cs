...
namespace Sitecore.Support.Data.Fields
{
    public class TemplateFieldSourceField : InternalLinkField
    {
        ...
        public override void ValidateLinks([NotNull] LinksValidationResult result)
        {
            ...
            switch (type)
            {
                ...
                case "grouped droplist":
                  ...
                  {
                    ValidateMultilistWithSearch(result, value);
                    return;
                  }
        ...
        /// <returns></returns>
        protected virtual string FilterTargetPath(string targetPath)
        {
        ...
        private void ValidateRules([NotNull] LinksValidationResult result)
        {
            ...
            this.ParseSource(options, value);
      ...
      internal void ParseSource(RulesEditorOptions options, string source)
      {
        ...
        if (!string.IsNullOrEmpty(source))
        {
          if (source.ToLowerInvariant().Contains("rulespath") || source.ToLowerInvariant().Contains("hideactions"))
          {
            ...
            string text = urlString["hideactions"];
            if (!string.IsNullOrEmpty(text))
            {
              options.HideActions = (string.Compare(text, "true", StringComparison.InvariantCultureIgnoreCase) == 0);
            }
          }
          else
          {
      ...
      void ValidateMultilistWithSearch([NotNull] LinksValidationResult result, string value)
      {
        Assert.ArgumentNotNull(result, "result");
        ...
        UrlString parameters = new UrlString(value);
        Item targetItem;
        string[] extraParams = new[] { parameters["datasource"], parameters["TemplateFilter"] };
        foreach (var param in extraParams)
        {
          if (string.IsNullOrEmpty(param))
            continue;
          targetItem = database.GetItem(param);
          if (targetItem != null)
          {
            result.AddValidLink(targetItem, value);
            break;
        ...
        void ValidateComplexLookupSource([NotNull] LinksValidationResult result, string value)
        {
            ...
            UrlString parameters = new UrlString(value);
          Item targetItem;
            string datasource = parameters["datasource"]; //StartSearchLocation TemplateFilter
            if (string.IsNullOrEmpty(datasource))
            {
              result.AddBrokenLink(value);
                return;
            ...
            targetItem = database.GetItem(datasource);
